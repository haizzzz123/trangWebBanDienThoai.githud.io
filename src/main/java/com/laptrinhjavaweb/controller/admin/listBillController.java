...
@WebServlet(urlPatterns = {"/admin-danh-sach-hoa-don"})
public class listBillController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String idUser= request.getParameter("idNick");
		List<listBillModel> listBill = listBillDAO.danhSachBill(idUser);
		request.setAttribute("listBill", listBill);
		
		request.setAttribute("acc", idUser);
		RequestDispatcher rd = request.getRequestDispatcher("/views/admin/listBill.jsp");
